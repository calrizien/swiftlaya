...
**Status: initial native port, not a production-validated replacement.** The portable Swift SDK has executable tests. The Core ML backend, checkpoint exporter, and native tokenizer integration are implemented but still require real-checkpoint conversion and macOS/Swift 6.4 validation. No converted weights, Apple-device benchmarks, or claims of accuracy parity are included. Training has not been rewritten in Swift.
...
| `LayaCoreML` | Full decision-model inference using local `.mlpackage` / `.mlmodelc` artifacts | macOS 14+, iOS 17+, visionOS 1+ |
...
This is the unverified part of the port. Use a local Laya checkpoint that includes `model.safetensors`, `rl_agent_config.json`, `encoder/config.json`, and `tokenizer/`.

```sh
python3 -m venv .venv
. .venv/bin/activate
python -m pip install -e . coremltools
python Tools/export_coreml.py \
  --checkpoint /absolute/path/to/laya-checkpoint \
  --output /absolute/path/to/exports/laya-english
```

Run this on macOS. The exporter attempts to preserve the entire encoder, type embedding, Transformer decision head, scorer, and action head. It checks several batch/sequence/option shapes against PyTorch and verifies the converted graph before setting `coreMLVerified`. It does not silently substitute an embedding model or synthetic output when conversion fails.

For an explicit Hub download, pass a repository id and `--revision` with an immutable model commit SHA. `--subfolder multilingual` or `--subfolder typed-decisions` selects the bundled variants. Model weights must be pinned independently of this code repository. The initial export defaults to eight questions and sixteen options per batch, with bounded flexible shapes; limits are recorded in the artifact.
